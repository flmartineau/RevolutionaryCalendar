...
function toRepublican(date) {
    const startYear = 1; //An I
    const endYear = 14; //An XIV

    for (let year = startYear; year <= endYear; year++) {
        for (let month = 1; month <= 13; month++) {
            let m = month % 14
            const daysInMonth = m === 13 ? 6 : 30; // Complementary days in month 13, otherwise 30
            for (let day = 1; day <= daysInMonth; day++) {
                const republicanDate = `${day} ${m === 13 ? 'complémentaire' : REPUBLICAN_MONTHS[m - 1]} An ${YEARS_ROMAN[year - startYear]}`;

                const correspondingGregorianDate = toGregorian(republicanDate);
